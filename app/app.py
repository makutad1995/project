...
def user(username):
    ...
    c.execute("SELECT name, body, date, score, Review.id, Review.id FROM  site.Review, site.beer WHERE beer.id = Review.beer_id AND userName LIKE %s ORDER BY date DESC", [thwart(username)])
    posts = c.fetchall()
    conn.commit()
    c.close()
    conn.close()
